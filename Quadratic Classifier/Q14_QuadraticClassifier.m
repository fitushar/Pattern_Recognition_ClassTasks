%% Problem 1.4. Quadratic Classifier
...
Error_QR=0;           % Defining Initial Error=0
TPR_QR=0;             % Defining Initial True Positive Rate(TPR)=0
AUC_QR=0;             % Defining Initial Area Under Curve(AUC)=0
%% Importing the Data
...
for iteration=1:numberOfitaration
    ...
    for i=1:length(Test_Data)
        ...
        g1(i,:)=discriminantFunction(Feature_vector,mean_PositiveClass,sigma_PositiveClass,P_of_PositiveClass); %%finding descriminat Function for the positive class for the ith sample
        g2(i,:)=discriminantFunction(Feature_vector,mean_NegativeClass,sigma_NegatieClass,P_of_NegativeClass); %%finding descriminat Function for the positive class for the ith sample
  ...
  %Evaluating desion and Storing the Decision Data in Test set at 12th colum
     ...
     if Test_Data(i,12)~= Test_Data(i,11)
          ErrorCounter_QR=1+ ErrorCounter_QR;  
     end
  ...
  [R_QR,AUC_QR]=EvalROC(ROC_Matrix,+1,-1); %Getting the coordinates for ROC Plot
   Q=[];
   RChPlot(R_QR,Q,'ROC: Quadratic Classifier'); %Ploting the ROC
  
%% At a FPR of 0.1, TPR is found from the ROC
FPR_index = find(R_QR(:,1)==0.1); %% Taking all the rows having FPR=0.1
TPR_QR = sum(R_QR(FPR_index,2))/length(FPR_index); %True Postive Rate for FPR=0.1
    

%% Stroing All the Results of each Iteration in a Matrix
Performance_Parameters_Matrix(1,iteration)=(length(Test_Data)-ErrorCounter_QR)/length(Test_Data); %% Accuracy
Performance_Parameters_Matrix(2,iteration)=TPR_QR; %%Storing TPR
Performance_Parameters_Matrix(3,iteration)= AUC_QR; %%Stoing AUC
